...
vector<int> kCheapestFlights(vector<vector<pair<int,int>>>& graph, int k) {
	int n = graph.size()-1;
	...
	while(!pq.empty() && results.size() < k) {
		...
		if(node == n) results.push_back(cost);
		
		if(count[node] > k) continue;
		

		for(auto [wt,neighbour]: graph[node]) {
		    pq.push({cost + wt, neighbour});
		}
	}
	return results;
}
...
int main()
{
	...
	vector<int> ans = kCheapestFlights(graph,3);
